...
<div class="container-fluid">
          ...
          <div class="col-12">
            <div class="card">
              <div class="card-header">
                <h3 class="card-title">Liste des volontaires</h3>
              </div>
              <!-- /.card-header -->
              <div class="card-body">
                    ...
                    <div class="col-sm-12">
                      <table id="example1" class="table table-bordered table-striped dataTable dtr-inline" aria-describedby="example1_info">
                        ...
                        @foreach($volontaires as $volontaire)
                          <tr class="even">
                            <td class="dtr-control sorting_1" tabindex="0">{{$volontaire->created_at}}</td>
                            <td>{{$volontaire->name}}  {{$volontaire->prenom}}</td>
                            <td>{{$volontaire->role}}</td>
                            <td>{{$volontaire->telephone}}</td>
                            <td>{{$volontaire->email}}</td>
                            <td class="project-actions text-right">
                            <a class="btn btn-primary btn-sm" 
                                  data-toggle="modal" data-target="#modal-lg{{$volontaire->id}}"
                                  href="{{route('detail', $volontaire->id)}} ">
                                  <i class="fas fa-eye">
                                  </i>
                                  Détails
                              </a>
    ...
    @foreach($volontaires as $volontaire)
      <div class="modal fade" id="modal-lg{{$volontaire->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
          <div class="modal-content">
            <div class="modal-header">
              <h4 class="modal-title">Détails du volontaire</h4>
              <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
              </button>
            </div>
            <div class="modal-body">
              <h5><span class="text-primary">Nom & Prénom :</span> <span style="font-size:15px;" class="text-justify ml-3">{{ $volontaire->name }} {{ $volontaire->prenom }}</span></h5>
              <h5><span class="text-primary">Profil :</span> <span style="font-size:15px;" class="text-justify ml-3">{{ $volontaire->role }}</span></h5>
              <h5><span class="text-primary">Téléphone :</span> <span style="font-size:15px;" class="text-justify ml-3">{{ $volontaire->telephone }} </span></h5>
              <h5><span class="text-primary">Email :</span> <span style="font-size:15px;" class="text-justify ml-3">{{ $volontaire->email }} </span></h5>
              <span class="float-right">Créer le : {{ $volontaire->created_at }} </span>
            </div>
